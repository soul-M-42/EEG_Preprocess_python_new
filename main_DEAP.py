...
for sub in tqdm(range(n_sub), desc=f'Reading sub:'):
    ...
    try:
        ...
        epochs = mne.Epochs(
            raw_bdf, 
            events, 
            event_id=4,  # 根据 trigger 划分试次
            tmin=0,  # 试次的开始时间
            tmax=60,   # 试次的结束时间
            baseline=(0, 0)
        )
    except:
        print(f'Sub {sub} cannot find event.')
    for trial_id in range(40):
        try:
            info = mne.create_info(ch_names=epochs.ch_names, sfreq=sfreq, ch_types='eeg')
            raw_i = mne.io.RawArray(epochs[trial_id].get_data()[0], info)
        ...
        try:
            ...
            print(f'selecting channel ERROR Sub {sub+1} Trial {trial_id}')
